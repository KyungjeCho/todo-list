...
- **OS Share Sheet 필터링**: "나에게 보내기" 지원 앱만 필터링하여 표시하는 기능은 OS API 제약으로 포함하지 않는다. 대신 앱 내 "클립보드 복사" 기능을 제공한다.
...
> **변경 사항 (2026-04-03):** 기존 "나에게 전송" 옵션을 삭제하고 "클립보드 복사" 기능으로 대체. 사용자가 공유 버튼을 탭하면 공유 채널 목록 최상단에 "클립보드 복사" 옵션이 표시되며, 탭 시 선택된 일정이 텍스트 형태로 클립보드에 복사되고 완료 토스트가 표시된다. OS Share Sheet의 "나에게 보내기" 필터링은 기술적으로 불가능하여 앱 내 클립보드 복사로 전환하였다.

![이월.png](attachment:c0ed0104-4ac7-4d4e-8ecc-06c080e4d2b5:이월.png)

사용자의 이월 기능 Sequence Diagram.

> **변경 사항 (2026-04-15, 008-update-01-ui-fixes):** 자정 이월 루틴은 어제 원본 todo의 `status`를 `CARRIED_OVER`로 전이하지 **않는다**. 원본은 `ACTIVE`/`COMPLETED`/`INACTIVE` 상태를 그대로 유지하며, 이월 중복 방지는 `TODOLIST_CARRIED_OVER_HISTORY.from_todo_id` UNIQUE 제약이 담당한다. "이월됨" 표시는 `GetTodosUsecase`가 `CarriedOverHistory`의 to-todo id 집합으로 도출한다. 기존 데이터에 남아 있는 `CARRIED_OVER` 레코드(레거시)는 호환성 유지를 위해 enum과 `VALID_TRANSITIONS` 상수를 제거하지 않는다.
