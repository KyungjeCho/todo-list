...
| APPLE_MUST_USE_FORM_POST | Apple은 GET 콜백 미지원 — `POST /auth/oauth/apple/callback` 사용 필요 |

---

### 1.2.1 Apple OAuth 콜백 (form_post 전용)

Apple은 `response_mode=form_post`를 사용하므로 GET 대신 `application/x-www-form-urlencoded` POST를 수신합니다. 첫 로그인 시에만 Apple이 `user`(JSON) 필드로 이름을 전달하며, 재로그인 시에는 포함되지 않습니다. 서버는 `id_token`을 JWKS로 검증해 `sub`·`email`을 확보하고, `client_secret`은 ES256 JWT로 동적 생성합니다.
...
| Param | Type | Required | Description |
| --- | --- | --- | --- |
| code | string | 필수 | Apple authorization code |
| state | string | 필수 | CSRF 방지용 HMAC 서명 state |
| user | string | 선택 | 첫 로그인 시 Apple이 전달하는 JSON (`{ "name": { "firstName": "...", "lastName": "..." } }`), 최대 4096자 |
| id_token | string | 선택 | 클라이언트 측 Apple SDK가 함께 전달 시 허용 (서버는 토큰 교환으로 재획득) |

**처리 흐름:**

```
1. state 검증 (CSRF, nonce, 디바이스 바인딩)
2. Apple token endpoint에 code + ES256 JWT client_secret 전송
3. 응답의 id_token을 JWKS로 검증 (iss/aud/exp/alg)
4. sub(providerUserId) + email로 UserOauth 조회
5. 기존 계정이면 isNewUser=false로 재로그인, 신규면 user JSON → user_name 생성
6. 재로그인 시 user.user_name 덮어쓰기 없음 (이름 보존)
7. JWT 발급 후 앱 딥링크로 fragment redirect
```

**Response 302:**

```
todolist://auth/callback#accessToken={jwt}&refreshToken={jwt}&isNewUser={bool}
```

**Errors (HTTP 400, `response.body.message`):**

| Code | 상황 |
| --- | --- |
| APPLE_CLIENT_SECRET_FAILED | p8 private key 로드 또는 ES256 JWT 생성 실패 |
| OAUTH_CODE_EXCHANGE_FAILED | Apple token endpoint non-2xx 또는 비 JSON 응답 |
| APPLE_ID_TOKEN_INVALID | id_token 부재·서명 실패·클레임(iss/aud/exp) 위반 |
| INVALID_OAUTH_STATE | state 서명·만료·디바이스 바인딩 검증 실패 |

**민감 정보 취급(SC-006):** `APPLE_PRIVATE_KEY`, `code`, `id_token`, `Authorization` 헤더는 로그·응답에 포함되지 않으며 Apple 응답 본문은 에러 시 선두 300자만 기록합니다.

**필요 환경 변수:**

| 변수 | 설명 |
| --- | --- |
| `APPLE_CLIENT_ID` | Apple Service ID (e.g., `com.todolist.service`) |
| `APPLE_TEAM_ID` | Apple Developer Team ID |
| `APPLE_KEY_ID` | AuthKey `.p8` 파일의 Key ID |
| `APPLE_PRIVATE_KEY_PATH` | `.p8` 파일 경로 (배포 환경에서 사용) |
| `APPLE_PRIVATE_KEY` | `.p8` 파일 내용 문자열 (환경 변수 주입 시 사용, path와 택일) |
| `APPLE_CALLBACK_URL` | Apple 콘솔에 등록된 POST 콜백 URL |
...
> `hasCompletedOnboarding`은 온보딩 완료 여부의 유일한 진실 소스이며, `planTime`/`reviewTime` 의 존재 여부와 독립적으로 관리됩니다 (007-fix-login-notif-calendar FR-001/FR-002).

---

### 2.2 프로필 설정 변경

사용자 프로필 및 루틴 시간을 변경합니다. (FR-16, FR-17)

> **온보딩 흐름:** 신규 유저는 `GET /users/me` 응답에서 `hasCompletedOnboarding === false` 입니다. 클라이언트는 온보딩 화면에서 ① `PATCH /users/me/settings` 로 시간/타임존/언어를 저장한 뒤, ② `POST /users/me/onboarding/complete` 를 호출해 완료 플래그를 TRUE 로 전이시킵니다. 시간 값과 완료 플래그는 구조적으로 분리되어, 알림 OFF(`planTime=null`) 해도 온보딩 상태가 되돌아가지 않습니다 (007-fix-login-notif-calendar).
...
온보딩 완료 플래그(`hasCompletedOnboarding`)를 TRUE 로 전이시키는 전용 엔드포인트입니다. `PATCH /users/me/settings` 와 분리되어, 알림 시간 변경이 온보딩 상태에 영향을 주지 않도록 보장합니다 (FR-001/FR-002).
...
**Response 200:** `UserProfile` (GET /users/me 와 동일 스키마, `hasCompletedOnboarding: true`)
...
> **멱등(idempotent):** 이미 `true` 상태에서 다시 호출해도 200 과 현재 프로필을 반환합니다. 추가 상태 전이는 일어나지 않으며, 중복 호출이 레이스 컨디션을 유발하지 않습니다.
...
| 404 | NOT_FOUND | 인증 토큰의 사용자를 찾을 수 없음 |
...
> **이월 동작 변경 (2026-04-15, 008-update-01-ui-fixes):** 어제의 원본 todo는 더 이상 `CARRIED_OVER`로 전이되지 않고 원래 상태(`ACTIVE`/`INACTIVE`/`COMPLETED`)를 그대로 유지합니다. 이월 여부는 응답의 `isCarriedOver` 필드로 판정하며, 서버는 `CarriedOverHistory.fromTodoId` UNIQUE 제약으로 중복 생성을 방지합니다. `CARRIED_OVER` enum 값은 과거 데이터 호환을 위해 유지됩니다.
...
| Auth | GET | /auth/oauth/:provider/callback | OAuth 콜백 (google/naver/kakao) | - | P0 |
| Auth | POST | /auth/oauth/apple/callback | Apple form_post 콜백 | - | P0 |
| Auth | POST | /auth/token/refresh | 토큰 갱신 | - | P0 |
| Auth | POST | /auth/logout | 로그아웃 | - | P0 |
| User | GET | /users/me | 프로필 조회 | - | P0 |
| User | PATCH | /users/me/settings | 프로필 설정 변경 | FR-16, FR-17 | P0 |
| User | POST | /users/me/onboarding/complete | 온보딩 완료 플래그 전이 (멱등) | FR-001, FR-002 | P0 |
