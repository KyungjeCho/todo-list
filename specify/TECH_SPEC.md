...
TodoList는 "계획 → 실행 → 회고" 루틴 기반의 Todo 일정 관리 앱이다. 사용자는 하나의 메인 페이지에서 시간대에 따라 Plan/Review 모드가 전환되며, 할 일 관리, 음성 인식 추가(LLM AI), 자동 이월, 캘린더 페이지, 공유 기능을 사용할 수 있다.
...
이 테크 스펙은 PRD에 정의된 17개 기능적 요구사항(FR-01 ~ FR-17)과 7개 비기능적 요구사항(NFR-01 ~ NFR-07)을 기술적으로 구현하기 위한 설계를 다룬다.
...
- STT, LLM API를 활용한 음성 인식 할 일 추가 기능을 구현한다.
...
사용자의 음성인식 및 LLM 요약 기능 Sequcence.
...
4. 서버에서 STT API로 STT 진행
5. 서버에서 LLM API로 텍스트 다듬기 처리
6. 다듬어진 할 일이 목록에 추가
7. 네트워크 오류 시: "네트워크 오류 안내"
```
...
- LLM API 키는 서버에서만 관리, 클라이언트에 노출하지 않음
...
- LLM API 타임아웃(5초) 초과 시 토스트 메시지로 안내

### 6.6 테스트 전략

- 단위 테스트: 이월 로직, 모드 전환 로직, 상태 변환, LLM 프롬프트 처리 등 핵심 비즈니스 로직 대상
