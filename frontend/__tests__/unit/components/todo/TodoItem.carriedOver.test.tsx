...
describe('TodoItem 이월 뱃지', () => {
  ...
  describe('isCarriedOver 뱃지 표시', () => {
    it('isCarriedOver가 true인 항목에 이월 뱃지를 표시한다', () => {
      ...
      expect(screen.getByTestId('carried-over-badge-todo-carried-1')).toBeTruthy();
    });

    it('isCarriedOver가 false인 항목에 이월 뱃지를 표시하지 않는다', () => {
      render(<TodoItem todo={normalTodo} />);

      expect(screen.queryByTestId('carried-over-badge-todo-normal-1')).toBeNull();
    });
  ...
  describe('이월 항목의 상태별 표시', () => {
    ...
    it('이월 후 완료된 항목에도 이월 뱃지를 표시한다', () => {
      ...
      expect(screen.getByTestId('carried-over-badge-todo-carried-2')).toBeTruthy();
    });
  ...
  describe('접근성', () => {
    it('이월 뱃지에 접근성 라벨이 있다', () => {
      ...
      const badge = screen.getByTestId('carried-over-badge-todo-carried-1');
      expect(badge.props.accessibilityLabel || badge.props['aria-label']).toBeTruthy();
    });
    ...
    it('이월 항목의 접근성 라벨에 이월 정보가 포함된다', () => {
      ...
      const label = item.props.accessibilityLabel || item.props['aria-label'] || '';
